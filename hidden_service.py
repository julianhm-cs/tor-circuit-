...
from dependency.torpy.ext.cells import CellRelayEstablishRendezvous, CellRelayIntroduce1, CellRelayRendezvousEstablished, CellRelayIntroduceAck
...
def extend_to_hidden(circuit, hidden_service):
    ...
    # logger.info('Pick a hidden service directory to look up possible introduction point')
    ...
    # logger.info('Pick an introduction point (router object) to the hidden service')
    ...
    # logger.info('And then also pick a rendezvous point of which to proceed with server requests/communications')
    ...
    # logger.info('Set up an introduction point for the hidden service')
    ...
    # logger.info('Add the introduction point as the next hop in the circuit')
...
if __name__ == '__main__':
    ...
    if args.outfile == "":
        ...
        outfile = open(args.outfile + "_" + str(run) + ".html", "w")
        outfile.write(response)
        outfile.close()
    # with open("time_" + str(run) + ".txt", "w+") as tf:
