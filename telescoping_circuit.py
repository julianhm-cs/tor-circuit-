...
def extend(circuit, node_router):
    ...
    timings.append('Circuit Extension Time: ' + str(end - start))
...
def circuit_build_hops(circuit, middle_router, exit_router):
    ...
    timings.append('Circuit Completion Time: ' + str(end - start))
    return circuit
...
def circuit_from_guard(guard_router, circuit_id):
    ...
    timings.append('Base Circuit Time: ' + str(end - start))
    return circuit


# Initiates the construction of a circuit and then gets a web page through that circuit
def get(hostname, port, path="", guard_address=None, middle_address=None, exit_address=None, live=False):
    start = time.perf_counter()
    request_template = b'GET /%s HTTP/1.0\r\nHost: %s\r\n\r\n'
    if not live:
        tor = TorClient()
    else:
        tor = TorClient(use_local_directories=False)
    ...
    middle = None
    exit = None
    if not guard_address and not middle_address and not exit_address:
        guard = random_router(get_all_relays(tor.consensus))
        while not middle or middle == guard:
            middle = random_router(get_all_relays(tor.consensus))
        while not exit or exit == guard or exit == middle:
            exit = random_router(get_all_exits(tor.consensus))
        pass
    else:
        guard = router_from_ip(guard_address, tor.consensus)
        middle = router_from_ip(middle_address, tor.consensus)
        exit = router_from_ip(exit_address, tor.consensus)
		
    circuit_base =  circuit_from_guard(guard, gen_circuit_id())  # CREATE

    circuit =  circuit_build_hops(circuit_base, middle, exit)  # EXTEND
    ...
    request = request_template % (path.encode(), hostname.encode())
    ...
    timings.append('Get Time: ' + str(end - start))
...
def main():
    ...
    timings.append('Total Response Time: ' + str(end - start))

    # Write response to args.outfile or stdout
    if args.outfile == "":
        print('response', response)
        # [print(x) for x in timings]
    else:
        outfile = open(args.outfile, "w")
        outfile.write(response)
        outfile.close()
    # with open("time_" + str(run) + ".txt", "w+") as time_file:
    #     [time_file.write(x + '\n') for x in timings]


if __name__ == '__main__':
    logger = logging.getLogger("")
    # register_logger(verbose=0, log_file='log_' + str(run) + '.log')
